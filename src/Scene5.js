...
var jumpLayer5 = cc.Layer.extend({
    size:null,
    ctor:function(){
        this._super();
		...
		Global.getInstance().nowscene = new jumpScene5();
		
		//背景图片
        var bgSprite = new cc.Sprite(res.Scene5_jpg);	
        bgSprite.attr({
            x:this.size.width/2,
		...
		//事件
		Scene_event.getInstance().set_event(this,2,day_num,group_name);
		//状态栏
...
var jumpScene5 = cc.Scene.extend({
    onEnter:function(){
      this._super();
      var layer = new jumpLayer5();
      this.addChild(layer);
    },
})
