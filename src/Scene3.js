...
var jumpLayer3 = cc.Layer.extend({
    size:null,
    ctor:function(){
        this._super();
		...
		Global.getInstance().nowscene = new jumpScene3();

		//背景图片
        var bgSprite = new cc.Sprite(res.Scene3_jpg);	
        bgSprite.attr({
            x:this.size.width/2,
		...
		//事件
		Scene_event.getInstance().set_event(this,3,day_num,group_name);
		//状态栏
...
var jumpScene3 = cc.Scene.extend({
    onEnter:function(){
      this._super();
      var layer = new jumpLayer3();
      this.addChild(layer);
    },
})
