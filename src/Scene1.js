...
var jumpLayer1 = cc.Layer.extend({
    size:null,
    ctor:function(){
	    this._super();
		...
		Global.getInstance().nowscene = new jumpScene1();
		//背景图片
        var bgSprite = new cc.Sprite(res.Scene1_jpg);	
        bgSprite.attr({
            x:this.size.width/2,
		...
		//事件
		Scene_event.getInstance().set_event(this,4,day_num,group_name);
		//状态栏
...
var jumpScene1 = cc.Scene.extend({
    onEnter:function(){
      this._super();
      var layer = new jumpLayer1();
      this.addChild(layer);
    },
})
